...
use crate::types::{Error, Vote, VotingConfig};
...
impl VotingSystem {
    pub fn cast_vote(
        ...
        env.events().publish((symbol_short!("vote"), symbol_short!("cast")), (
            proposal_id,
            voter,
            support,
            weight
    ...
    fn get_vote_key(env: &Env, proposal_id: u32, voter: &Address) -> Symbol {
        Symbol::new(&env, &format!("VOTE_{}:{:?}", proposal_id, voter))
    }

    fn get_vote_totals_key(env: &Env, proposal_id: u32, support: bool) -> Symbol {
        Symbol::new(&env, &format!("VOTE_{}:{}", proposal_id, if support { "FOR" } else { "AGAINST" }))
    }
