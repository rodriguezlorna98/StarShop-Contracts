...
impl AdminModule {
    ...
    pub fn set_points_expiry(env: &Env, days: u64) -> Result<(), Error> {
        ...
        env.storage().instance().set(&DataKey::PointsExpiryDays, &days);
    ...
    pub fn set_points_ratio(env: &Env, ratio: u32) -> Result<(), Error> {
        ...
        // Update points ratio
        env.storage().instance().set(&DataKey::PointsPerPurchaseRatio, &ratio);
        
        Ok(())
    }
    
    /// Set bonus points for a product category
    pub fn set_category_bonus(env: &Env, category: &soroban_sdk::Symbol, bonus_bps: u32) -> Result<(), Error> {
        // Verify admin
        Self::verify_admin(env)?;
    ...
    pub fn set_product_bonus(env: &Env, product_id: &soroban_sdk::Symbol, bonus_bps: u32) -> Result<(), Error> {
        // Verify admin
        Self::verify_admin(env)?;
